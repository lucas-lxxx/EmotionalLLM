...
@dataclass
class Config:
    # Paths - testN10 experiment configuration
    ...
    sample_list_path: Path = Path(__file__).resolve().parent / "sample_list.txt"
    results_dir: Path = Path(__file__).resolve().parent / "results"

    # Runtime - Use cuda:6 as specified
    device: str = "cuda:0"  # Will be mapped via CUDA_VISIBLE_DEVICES=6
    seed: int = 1234

    # Prompts (Methodology §3)
    system_prompt: str = "You are a helpful assistant."
    emo_labels: list[str] = field(default_factory=lambda: ["happy", "sad", "angry", "neutral"])
    emo_prompts: list[str] = field(
        default_factory=lambda: [
            ...
            "Emotion label only (one word): happy, sad, angry, or neutral.",
        ]
    )
