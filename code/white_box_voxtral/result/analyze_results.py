#!/usr/bin/env python3
"""
Voxtral 白盒对抗攻击实验结果批量清洗、统计与报告生成脚本
========================================================
复用 ESDfinal analyze_results.py 的完整分析流水线，
自动处理 Voxtral_EN 和 Voxtral_CN 两个子集。
...
def llm_judge_batch(
    ...
    system_prompt = (
        "你是一个语义一致性裁判员。给定两段文本（原始文本 A 和对抗文本 B），"
...
def generate_report(
    ...
    records: list[dict[str, Any]],
    output_path: Path,
    title: str = "Voxtral 白盒对抗攻击实验统计报告",
):
    """生成 Markdown 格式的统计报告。"""
...
def analyze_subset(
    ...
    """对单个子集 (Voxtral_EN / Voxtral_CN) 执行完整分析流水线。"""
    print(f"\n{'='*60}")
    ...
    # 加载与清洗
    print(f"[1/5] 加载与清洗数据...")
    records, warnings = load_and_clean(subset_dir)
    if not records:
        print(f"[WARN] {subset_name} 无有效数据，跳过")
        return records, warnings
    print(f"  加载 {len(records)} 条记录，{len(warnings)} 条警告")

    # LLM Judge
    if llm_judge and api_key:
        print(f"[2/5] 运行 LLM Judge 语义一致性评估...")
        records = llm_judge_batch(records, api_key, model, base_url)
    else:
        print(f"[2/5] LLM Judge 未启用")
...
def main():
    parser = argparse.ArgumentParser(description="Voxtral 实验结果分析")
    ...
    # 自动发现子集目录 (Voxtral_EN, Voxtral_CN, ...)
    subset_dirs = sorted([
    ...
    for subset_dir in subset_dirs:
        records, warnings = analyze_subset(
            subset_dir, subset_dir.name,
        ...
        # 为合并报告添加 subset 标记
        for r in records:
            r["subset"] = subset_dir.name
        all_records.extend(records)
        all_warnings.extend([f"[{subset_dir.name}] {w}" for w in warnings])

    # 合并总报告
    if len(subset_dirs) > 1 and all_records:
        print(f"\n{'='*60}")
        print(f"  生成合并总报告")
        print(f"{'='*60}")

        print(f"[3/5] 计算合并统计指标...")
        combined_stats = compute_stats(all_records)

        csv_path = result_dir / "cleaned_data_all.csv"
        print(f"[4/5] 导出合并 CSV: {csv_path}")
        export_csv(all_records, csv_path)

        report_path = result_dir / "report_all.md"
        print(f"[5/5] 生成合并报告: {report_path}")
        generate_report(combined_stats, all_warnings, all_records, report_path,
                        title="Voxtral 白盒对抗攻击实验统计报告（EN + CN 合并）")

    print(f"\n[DONE] 全部分析完成！")
    for subset_dir in subset_dirs:
        print(f"  - {subset_dir.name}: {subset_dir / 'cleaned_data.csv'}, {subset_dir / 'report.md'}")
    if len(subset_dirs) > 1:
        print(f"  - 合并: {result_dir / 'cleaned_data_all.csv'}, {result_dir / 'report_all.md'}")
