...
from meralion_io import build_inputs, forward_logits
...
def loss_emo(
    ...
    for prompt in prompts:
        inputs = build_inputs(
            waveform, sr, prompt, processor, device,
            torch_extractor=torch_extractor,
            differentiable=True, dtype=dtype,
        )
...
def attack_one_sample(
    ...
    for step in range(cfg.total_steps):
        ...
        for eot_idx in range(cfg.eot_samples):
            ...
            l_emo = loss_emo(
                model, processor, waveform_adv, sr,
                cfg.emo_prompts, target_token_ids,
                device, ignore_index, torch_extractor, model_dtype,
            )
            l_asr = loss_asr(
                model, processor, waveform_adv, sr,
                asr_prompt, asr_target_token_ids,
                device, ignore_index, torch_extractor, model_dtype,
            )
        ...
        if grad_norm < cfg.grad_norm_min:
            ...
            if small_grad_steps >= cfg.grad_norm_patience:
                raise RuntimeError("Grad norm too small; check gradient chain.")
        else:
            small_grad_steps = 0
