"""Qwen-Omni API client for audio emotion detection (supports multiple Qwen models)."""
...
class QwenClient:
    def __init__(self, api_key: str | None = None, model: str | None = None):
        ...
        self.model = model or cfg.qwen3_model
        if not self.api_key:
            raise ValueError("DASHSCOPE_API_KEY not set")

        from openai import OpenAI
        self.client = OpenAI(api_key=self.api_key, base_url=cfg.qwen_base_url)
    ...
    def _detect_format(self, audio_path: Path) -> str:
        ...
        return {".wav": "wav", ".mp3": "mp3", ".flac": "flac"}.get(suffix, "wav")

    def query_emotion(self, audio_path: Path, prompt: str) -> str:
        """Send audio + prompt to Qwen-Omni."""
        ...
        messages = [{
            ...
            "content": [
                {
                    "type": "input_audio",
                    "input_audio": {"data": f"data:;base64,{audio_b64}", "format": audio_fmt},
                },
